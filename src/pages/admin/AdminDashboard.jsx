  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const [staffRes, appointmentRes, patientsRes] = await Promise.all([
          axios.get(`${API_URL}/users?role=staff${facilityId ? `&facilityId=${facilityId}` : ''}`, { 
            headers: { Authorization: `Bearer ${token}` } 
          }),
          axios.get(`${API_URL}/appointments/stats${facilityId ? `?facilityId=${facilityId}` : ''}`, { 
            headers: { Authorization: `Bearer ${token}` } 
          }),
          axios.get(`${API_URL}/patients/stats${facilityId ? `?facilityId=${facilityId}` : ''}`, { 
            headers: { Authorization: `Bearer ${token}` } 
          }).catch(() => ({
            // Fallback data if endpoint doesn't exist yet
            data: { total: 0, inpatients: 0, outpatients: 0 }
          }))
        ]);
        ...
        setStats({
          ...
          inpatientCount: patients.inpatients || 0,
          outpatientCount: patients.outpatients || 0
        });
            ...
            <Grid item xs={6} md={3}>
              <PatientStatsWidget 
                ...
                stats={{
                  ...
                  inpatientCount: stats.inpatientCount || 0,
                  outpatientCount: stats.outpatientCount || 0
                }} 
              />
            </Grid>
