...
export default function Registration() {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    facilityIds: [], // Facility selection (multi-select)
  ...
  const validateForm = () => {
    ...
    if (!formData.facilityIds || formData.facilityIds.length === 0) {
      newErrors.facilityIds = 'Please select at least one hospital or clinic';
    }
    setErrors(newErrors);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        registrationType,
        role: 'PATIENT',
        facilityIds: formData.facilityIds, // Send array of facilities
      };
      ...
      <Container maxWidth="sm">
        <Paper elevation={6} sx={{ borderRadius: 4, p: { xs: 2, sm: 6 }, boxShadow: 6 }}>
          <form onSubmit={handleSubmit} style={{ width: '100%' }}>
            ...
            {/* Facility Selection */}
            <Grid item xs={12}>
              <FormControl fullWidth sx={formStyles.inputField}>
                <InputLabel id="facility-label">Hospital/Clinic(s)</InputLabel>
                <Select
                ...
                {errors.facilityIds && <Alert severity="error">{errors.facilityIds}</Alert>}
              </FormControl>
            </Grid>
